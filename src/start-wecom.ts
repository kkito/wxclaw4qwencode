 ...
 *
 * 配置：
 *   ~/.ownclaw/config.json → channel.wecom.botId / secret
 *
 * 用法：
 *   pnpm run build
 *   node dist/start-wecom.js
 */
...
import { WecomBridge } from './channel/wecom-bridge.js';
...
async function startWecom(): Promise<void> {
  ...
  const wecomConfig = channelConfig.wecom;
  if (!wecomConfig?.enabled || !wecomConfig.botId || !wecomConfig.secret) {
    console.error('企业微信通道未启用或缺少配置');
    process.exit(1);
  }
  ...
  logger.info('🚀 正在启动企业微信通道...');
  logger.info(`📡 模型: ${config.agentscope.model.modelName}`);
  logger.info(`🤖 Bot ID: ${wecomConfig.botId}`);
...
`;
  ...
  try {
    ...
    logger.error('企业微信 Bridge 启动失败:', error);
    process.exit(1);
  }
  ...
  const acpTimeoutCheck = setInterval(() =>
    acpManager.checkTimeouts(async (_userId, _msg) => {
      // WeCom 超时通知通过 bridge 发送
    }),
    60000,
  );

  logger.info('✅ 企业微信通道已就绪');
